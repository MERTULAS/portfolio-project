...
class Transform {
  ...
  shearX(shifter) {
    ...
    this.translate(
      -(newCenter[0] - tempCenter[0]),
      -(newCenter[1] - tempCenter[1])
    );
  }
...
class Triangle extends Transform {
  constructor(...args) {
    ...
    /* 
        ...
        */
    if (args.length !== 6)
      throw "You must give parameters like a this format => new Triangle(x1, y1, x2, y2, x3, y4)";
...
class Polygon extends TransformPolyPoints {
  ...
  get center() {
    ...
    return new Point(
      xCenter / this.points.length,
      yCenter / this.points.length
    );
  }
}

export { Triangle, Rectangle, Polygon };

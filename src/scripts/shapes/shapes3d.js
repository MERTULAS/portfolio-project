...
class Transform3D {
  projectionTo2D() {
    ...
    Object.keys(this._normalizedPoints).forEach((key) => {
      ...
      projectionResult = Vector.dot(
        [this._normalizedPoints[key].asArray],
        projectorMatrix
      );
      this.projectedPoints[key] = new Point(
        projectionResult[0][0],
        projectionResult[0][1]
      );
    });
  ...
  scale(scalarMultiple) {
    ...
    Object.keys(this.projectedPoints).forEach((key) => {
      scaledMatrix = Vector.dot(
        [this.projectedPoints[key].asArray],
        multiplerMatrix
      );
  ...
  normalize() {
    Object.keys(this.points).forEach((key) => {
      this._normalizedPoints[key] = new Point3D(
        ...Vector.normalize(this.points[key].asArray)
      );
    });
  }
}
...
class Cube extends Transform3D {
  /*
    ...
    */
  constructor(cubeCenter, scale, context) {
    super();
    this.ctx = context;
    this._scale = scale;
    this.points = {
  ...
  draw() {
    ...
    for (let i = 0; i < 4; i++) {
      this.ctx.moveTo(...projectedPointsAsArray[i].asArray);
      this.ctx.lineTo(...projectedPointsAsArray[(i + 1) % 4].asArray);

      this.ctx.moveTo(...projectedPointsAsArray[i + 4].asArray);
      this.ctx.lineTo(...projectedPointsAsArray[((i + 1) % 4) + 4].asArray);

      this.ctx.moveTo(...projectedPointsAsArray[i].asArray);
      this.ctx.lineTo(...projectedPointsAsArray[i + 4].asArray);
    }
    this.ctx.stroke();
  }
